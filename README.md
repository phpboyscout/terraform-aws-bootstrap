...
| <a name="input_nuke_blocklist"></a> [nuke\_blocklist](#input\_nuke\_blocklist) | Account IDs aws-nuke must NEVER target, written into the generated config's blocklist:. Defaults to the ekristen placeholder (["000000000000"]); set real production / shared-services account IDs when this config governs an account that shares an operator with others. | `list(string)` | <pre>[<br/>  "000000000000"<br/>]</pre> | no |
